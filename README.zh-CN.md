...
**为什么不让 agent 跑在第二个桌面或虚拟显示器上，这样它永远碰不到我的屏幕？**

因为 Windows 没有能用来实现它的机制，而手机那套之所以成立，靠的正是 Windows 缺的那一环。
Android 上应用可以创建 `VirtualDisplay`，并且**输入事件带 display id**，于是 agent 的点击被
路由到自己那块屏，人的触摸屏完全无感。**Windows 的输入是按「桌面」而不是按「显示器」分配的**：
一个桌面只有一条输入队列、一个光标位置。
...
| 想法 | 为什么隔离不了 |
|---|---|
| 加一块虚拟显示器（间接显示驱动） | 只是多一块画布，不是多一个光标——光标在所有显示器上仍然只有一个位置 |
| Windows 虚拟桌面（`Win+Ctrl+D`） | 同一个会话里的视图切换，同一条输入队列，同一个光标 |
| 第二个会话（RDP，或另开一个用户） | 隔离是真的，但客户端 Windows 同一用户同时只能有一个交互会话——远程连进去本地就锁屏，人反而没了屏幕，而那正是目的 |
| 隐藏的 Win32 桌面（`CreateDesktop`） | agent 确实会拿到自己的输入队列和光标，但它的窗口用户看不见，只能操作自己启动的实例，碰不到你正在看的那个程序。（按窗口站/桌面模型推断，此处未实测。） |

所以这个冲突不是本实现没做好，而是系统约束：**Windows 上不存在第二个光标。** 既然没有，
让行就是唯一正确的应对——而且大多数调用根本走不到这个冲突上，因为它们**完全不注入输入**：
...
**没有元素可以寻址**：`tool_click_at` 的裸事件路径，以及 `tool_send_keys` 的全局热键。
仲裁器守的就是这两条。
